...
'@
        ...
        Write-Verbose ("Adding items to image. Wait at least 1 min, then if new lines below dont appear press Enter - It sometimes hangs.")

        foreach($sourceItem in $sourceItems) {
			
            try {
                $image.Root.AddTree($sourceItem.FullName, $true)
				Write-Verbose ("Adding $sourceItem")
            } # try
            catch {
        ...
        try {
			...
			Write-Verbose ("Target File Created:" + $targetFile + " Starting to write contents, this can take several minutes... Press Enter to update in case it hangs." )
            [ISOFile]::Create($targetFile.FullName,$result.ImageStream,$result.BlockSize,$result.TotalBlocks)
			Write-Verbose ("Writing out to ISO file:" + $targetFile + " completed." )
        } # try
        catch {
