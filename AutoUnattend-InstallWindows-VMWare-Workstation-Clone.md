...
```
PS C:\WINDOWS\system32> Export-WindowsImage -SourceImagePath "C:\mount_Win10_22H2_x32_ISO\sources\install.esd" -SourceIndex 1 -DestinationImagePath "C:\mount_Win10_22H2_x32_ISO\sources\install.wim" -DestinationName "Windows 10 Home" -LogLevel debug -LogPath "C:\mount_Win10_22H2_x32_ISO\sources\install.wim.log" -CompressionType max -CheckIntegrity
```

As this is a new ```WIM``` file, its relative index position will be 1, if you were to export more images to the destination ```WIM``` file its relative index position will increase by 1. Contrary to the [documentation](https://learn.microsoft.com/en-us/powershell/module/dism/export-windowsimage?view=windowsserver2025-ps#-loglevel), the ```-loglevel 4``` is not accepted but ```debug``` can be used, taken from ```"Errors,Warnings,WarningsInfo,Debug"``` BUT, using debug only shows two lines, seen below, and ```-loglevel x``` doesnt output any real log info in practice.
...
| | Header Type | C:\Program Files | C:\Program Files (x86) | 
|--- | --- | --- | --- |
| 32bit OS | ```PE L``` | %ProgramFiles% | N/A |
| 64bit OS | ```PE d†``` | %ProgramFiles% | %ProgramFiles(x86)% |
...
## The ADK will only work when running it on the version of Windows you are trying to build an AutoUnattend.xml file for. 

## In otherwords, you cant build a Windows 10 AutoUnattend.xml on Windows 11 and vice versa.
...
| <UseConfigurationSet>true</UseConfigurationSet> | install.wim</Path> | No Images are available |
| <UseConfigurationSet>true</UseConfigurationSet> | install.esd</Path> | No Images are available |
...
```
The $OEM$ folders and D:\Sources folder get copied to C:\Windows when installation takes place. This can add 10GB to the VMware virtual PC taking up unneccesary space. This pushes you towards having to use the SysPrep method to create a minimal and thus reduced attack vector instance of Windows. 
