...
```.WIM``` files only [capture/contain a single partition, typically the Windows partition](https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/capture-and-apply-windows-using-a-single-wim), whereby other partitions are then configured and setup with data. ```FFU``` [captures the entire hard disk and all partitions on the hard disk](https://learn.microsoft.com/en-us/windows-hardware/manufacture/desktop/capture-and-apply-windows-system-and-recovery-partitions). 
