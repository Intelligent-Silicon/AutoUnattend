...
User App installation software can also be added for installation after Windows is installed, which can help with building an offline USB mem stick or ISO image file installation, for such scenerios like working remotely or in locations without internet access, but still need ALL software to be configured to run out-of-the-box when internet access or return to work occurs.
