...
Edit the ```.ISO``` or ```.VHD``` image file in ```C:\mount```. Treat this as if you were editing the USB mem stick created by the [Windows Media Creation Tool](https://support.microsoft.com/en-gb/windows/create-installation-media-for-windows-99a58364-8c02-206f-aa6f-40c3b507420d), eg load the ```.WIM``` or  ```.ESD``` files using DISM to add driver packages, add the ```AutoUnattend.xml``` file and optionally add drivers or software  and other software before then saving ```C:\mount``` to a new ```.ISO``` or ```.VHD``` file.
