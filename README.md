...
```.ESD``` image files is an encrypted version of the ```.WIM``` image file. 

As at 20250613:YYYYMMDD, DISM using "WimFltr v2 extractor", with the command ```Dism /add-package``` still fails to add Window's Update ```.MSU``` packages to the ```.ESD``` windows image file. 
...
To find out what version of Windows and its variants are stored in a ```.WIM``` or ```.ESD``` file, [Load Powershell as Administrator](https://learn.microsoft.com/en-us/powershell/scripting/windows-powershell/starting-windows-powershell?view=powershell-7.5#run-from-the-start-menu) type the following: 
...
To save the finished ```C:\mount``` folder along with its files and subfolders, in order to make it a Windows installation ```.ISO``` or ```.VHD``` image file, download and run the [New-ISOFile powershell module script](New-ISOFile.psm1) (To Fork Properly - https://thedotsource.com/2021/03/16/building-iso-files-with-powershell-7/) to create the resultant ```.ISO``` file. 
...
The Support URL format is<br/>```https://support.microsoft.com/help/[Number of the KB]```<br/> 
eg [https://support.microsoft.com/help/5063060](https://support.microsoft.com/help/5063060)
...
The Security Update [KB5060842](https://support.microsoft.com/help/5060842#:~:text=To%20install%20this%20update,%20use%20one%20of%20the%20following%20Windows%20and%20Microsoft%20release%20channels.&text=Catalog) can be installed to the Windows ```install.esd``` image file using DISM because it displays the instructions to do so. Scroll down to [Install Update](https://support.microsoft.com/help/5060842#:~:text=To%20install%20this%20update,%20use%20one%20of%20the%20following%20Windows%20and%20Microsoft%20release%20channels.&text=Catalog) to see. 
