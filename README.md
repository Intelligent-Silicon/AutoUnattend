...
In the example below, the powershell module is imported for use before being executed in order to make the ```ISO``` image file. The module is only available for the lifetime of the session (powershell window) it was imported into and is not available in new powershell windows afterwards.
