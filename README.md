...
```.ESD``` image files is an encrypted version of the ```.WIM``` image file. As at 20250613:YYYYMMDD, DISM using "WimFltr v2 extractor", ```Dism /add-package``` still fails to add Window's Update ```.MSU``` packages to the ```.ESD``` windows image file. 
